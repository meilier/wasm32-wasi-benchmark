...
# function benchmark_ssvm_test() {
#     echo benchmark_ssvm
#     for ((i=0; i<"${#NAME[@]}"; ++i)); do
#         for ((j=0; j<"${#ARGS[@]}"; ++j)); do
#             time "$SSVM" --enable-simd benchmark/ssvm/"${NAME[i]}".so "${ARGS[j]}" <benchmark/random >&/dev/null
#         done 2> shenmewanyi
#     done
# }
# benchmark_ssvm_test
function benchmark_ssvm2() {
    ...
    for ((i=0; i<"${#NAME[@]}"; ++i)); do
        ...
        /usr/bin/time -o "mengbile.time" --verbose "$SSVM" --enable-simd benchmark/ssvm/"${NAME[i]}".so "${ARGS[i]}" <benchmark/random >&/dev/null
    done
}
benchmark_ssvm2
...
function benchmark_lucet2() {
    echo benchmark_lucet
    for ((i=0; i<"${#NAME[@]}"; ++i)); do
        LOG="lucet-meng$i.log"
        for ((j=0; j<"${#ARGS[@]}"; ++j)); do
            time "$LUCET_WASI" benchmark/lucet/"${NAME[i]}".so "${ARGS[j]}" <benchmark/random
        done 2> "$LOG"
        /usr/bin/time -o "lucetmeng$i.time" --verbose "$LUCET_WASI" benchmark/lucet/"${NAME[i]}".so "${ARGS[i]}" <benchmark/random >&/dev/null
    done
}
#benchmark_lucet2
